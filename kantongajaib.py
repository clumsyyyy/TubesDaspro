...
def login(): #F01: LOGIN
    ...
    hashedpw = hashfunc.hashing(pw) #hashedpw adalah hasil hashing dari password yang diinput yang akan dicocokkan di user.csv
...
def register(): #FO2: REGISTER
    ...
    if doesNameExist == False: #jika tidak ada username yang baru, entry baru akan ditambahkan ke dalam user.csv dengan tipe akun user
        data_user.append([str(count), new_name, new_user, hashfunc.hashing(new_pw), new_alamat, "user"])
